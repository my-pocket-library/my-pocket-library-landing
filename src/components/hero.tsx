  ...
  return (
    <section className="relative isolate min-h-screen overflow-hidden bg-black">
      ...
      <div className="pointer-events-none relative z-10 mx-auto flex max-w-[1100px] flex-col items-center px-6 pt-44 text-center md:pt-52">
        <h1 className="font-serif text-balance text-5xl font-medium leading-[1.05] tracking-[-0.02em] text-white md:text-7xl">
        ...
        <div className="pointer-events-auto mt-8 flex items-center gap-3">
          <Button
            size="lg"
            className="group h-12 rounded-full bg-white px-6 text-[15px] font-medium text-black shadow-[0_2px_30px_-2px_rgba(255,255,255,0.25)] hover:bg-white/90"
          >
            <span className="mr-2 inline-block size-1.5 rounded-full bg-black" />
            Get the app
          </Button>
          <Button
            size="lg"
            variant="ghost"
            className="h-12 rounded-full px-6 text-[15px] text-white hover:bg-white/10 hover:text-white"
          >
            See how it works
          </Button>
        </div>
      ...
      >
        ...
        >
          ...
          {Array.from({ length: 40 }).map((_, i) => {
            ...
            return <circle key={`d-${i}`} cx={x} cy={y} r="2" fill="white" />;
          })}
        </g>
      </svg>
    </div>
